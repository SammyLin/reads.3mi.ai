...
草稿寫完、送出前，套用 `.claude/skills/speak-human-tw` 這個 skill 過一次。這是排程任務、沒有人在場勾選清單，走它自己定義的「自動化工作流模式 → 跳過確認、事後摘要」：不用列清單等回覆，步驟 1–5 分析出的建議全部直接套用到 `content_md`，跑完摘要開頭先講總數（「這次找到並修改了 N 處」），沒有要改的地方也要明講。

摘要留著，併進第 6 節的回報，不要跑完就丟掉。

`speak-human-tw` 不做事實查核，也不補你沒寫的細節——它只砍 AI 腔、抓中國用語，改完一樣要过一次「保真回讀」：保護清單五類（價格數字、專有名詞、網址、真名引號原話、承諾條款）有沒有被動到、原文每個事實點還在不在。
...
四句話：挑了哪篇、拆出的尺叫什麼、跳過了什麼以及為什麼、文章網址。附上第 4.5 節 `speak-human-tw` 的事後摘要（改了幾處、大致是哪幾類）。
